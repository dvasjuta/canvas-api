...
public class AccountImpl extends BaseImpl<Account, AccountReader, CanvasWriter> implements AccountReader {
    ...
    public Map<String, Boolean> listMyPermissions() throws IOException {
        LOG.debug("Getting list of permissions for the user making this call");
        String url = buildCanvasUrl("accounts/self/permissions", Collections.emptyMap());		
		Response response = canvasMessenger.getSingleResponseFromCanvas(oauthToken, url);
		...
		for (String key : object.keySet()) {
			permissions.put(key, Boolean.parseBoolean(object.get(key).getAsString()));
		}
		return permissions;
	}
