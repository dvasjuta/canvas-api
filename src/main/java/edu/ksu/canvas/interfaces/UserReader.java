...
public interface UserReader extends CanvasReader<User, UserReader> {
	 ...
	 *
	 * @param userIdentifier The object holding a user identifier value, for
	 * example: Numeric Canvas user ID A string like "sis_integration_id:123" to
	 * query by SIS integration ID A string like "sis_user_id:ABC" to query by
	 * SIS ID The special string "self" that the Canvas API interprets as "the
	 ...
	 * <ul>
	 * <li><code>search_term</code> - what is being used to search for
	 * users</li>
	 * <li><code>sort</code> - what order the users are coming back in;
	 * acceptable values here are <code>username</code>, <code>email</code>,
	 * <code>sis_id</code> and <code>last_login</code></li>
	...
	/**
	 * Retrieve custom data for user based on scope and namespace
	 *
	 * @param options API options for this API call
	 * @return JSON string of data
	 * @throws IOException When there is an error communicating with Canvas
	 */
	Optional<String> getUserCustomData(GetUserCustomDataOptions options) throws IOException;
}
